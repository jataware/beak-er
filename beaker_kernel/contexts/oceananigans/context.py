...
class OceananigansContext(BaseContext):
    ...
    async def setup(self, config, parent_header):
        ...
        load_commands = [
            '%s = parse_json_acset(SummationOceananigan
        {Symbol, Symbol, Symbol},"""%s""")' % (var_name, fetch_model(oceananigan_id))
            for var_name, oceananigan_id in config.items()
        ]

        command = "\n".join(
            [
                self.get_code("setup"),
                "oceananigan = @oceananigan begin end",
                *load_commands
            ]
        )
    ...
    async def auto_context(self):
        return """You are an scientific modeler whose goal is to construct a DecaExpr for Oceananigan
    s.jl modeling library.

Explanation of Oceananigan
    s.jl from the docs
> Discrete Exterior Calculus Applied to Partial and Ordinary Differential Equations (Oceananigan
    s) is a diagrammatic language
> used to express systems of ordinary and partial differential equations. The Oceananigan
...
> with them. Below, we provide a high-level overview of how Oceananigan
    s can be generated and interpreted.
...
--- START ---
{self.oceananigans_expression_dsl}
--- END ---
...
"""
    ...
    async def compile_expr(self, server, target_stream, data):
        ...
        self.oceananigans_expression_dsl = declaration
    ...
    async def construct_amr(self, server, target_stream, data):
        ...
        header =  {
            ...
            "_type": "Header",
            "model_version": "v1.0",
            "schema": "https://raw.githubusercontent.com/DARPA-ASKEM/Model-Representations/oceananigans-intertypes/oceananigans/oceananigans_schema.json",
            "schema_name": "oceananigan"
        }
